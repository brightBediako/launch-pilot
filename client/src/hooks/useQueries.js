...
export const useUpdateLaunch = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['launches', variables.launchId] });
      queryClient.invalidateQueries({ queryKey: ['launches'] });
    },
  });
};
...
export const useCreateTask = () => {
  ...
  return useMutation({
    mutationFn: async ({ launchId, taskData }) => {
      const response = await apiClient.post(`/launches/${launchId}/tasks`, taskData);
      return response.data.data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['tasks', variables.launchId] });
    },
  });
};
...
export const useGenerateContent = () => {
  return useMutation({
    mutationFn: async ({ launchId, contentType, context }) => {
      const response = await apiClient.post(`/launches/${launchId}/generate-content`, {
        contentType,
        context,
      });
      return response.data.data;
